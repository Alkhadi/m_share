...
class BankButtons extends StatelessWidget {
  const BankButtons({super.key, required this.profile, this.foreground});
  final Profile profile;

  /// Optional colour used for button text and border.  When provided,
  /// each button will adopt this colour, allowing the caller to adapt
  /// the appearance of the bank buttons to various backgrounds.  If
  /// null, the default OutlinedButton styling is used.
    ...
    return Wrap(
      ...
      children: banks.map((bank) {
        ...
        return OutlinedButton(
          onPressed: () => _openBank(context, bank),
          style: OutlinedButton.styleFrom(
            foregroundColor: fg,
            side: fg != null ? BorderSide(color: fg) : null,
          ),
          child: Text(bank),
        );
      }).toList(),
